...
namespace Browser
{
	...
	public partial class FormBrowser : Form, IBrowser
	{
		...
		public FormBrowser(string serverUri)
		{
			...
			// スクリーンショットプレビューコントロールの追加
			{
				...
				using (var g = Graphics.FromImage(control.Image))
				{
					...
					g.DrawString("还没有截过图。\r\n", new Font("Microsoft YaHei", 12, FontStyle.Regular, GraphicsUnit.Pixel), Brushes.Black, new Point(4, 4));
				}
		...
		private void ToolMenu_Other_ClearCache_Click(object sender, EventArgs e)
		{
			...
			if (MessageBox.Show("即将清除浏览器 (IE) 缓存。\n确定要清除吗？", "清除缓存", MessageBoxButtons.OKCancel, MessageBoxIcon.Question)
				== System.Windows.Forms.DialogResult.OK)
			{
				...
				BeginInvoke((MethodInvoker)(() =>
				{
					...
					if (succeeded)
						MessageBox.Show("缓存清除完成。", "清除完成", MessageBoxButtons.OK, MessageBoxIcon.Information);
					else
						MessageBox.Show("由于消耗时间过长，清除缓存操作已中断。\r\n存在没有清除彻底的可能性，建议手动清除 IE 缓存并刷新页面。", "操作中断", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
				}));
