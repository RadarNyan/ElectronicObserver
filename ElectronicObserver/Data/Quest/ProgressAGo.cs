...
namespace ElectronicObserver.Data.Quest
{
	...
	public class ProgressAGo : ProgressData
	{
		...
		public override string ToString()
		{
			...
			{
				new DSPair(Math.Min((double)sortieCount / sortieMax, 1.0), string.Format("出击 {0}/{1}", sortieCount, sortieMax)),
				new DSPair(Math.Min((double)sWinCount / sWinMax, 1.0), string.Format("S 胜 {0}/{1}", sWinCount, sWinMax)),
				new DSPair(Math.Min((double)bossCount / bossMax, 1.0), string.Format("BOSS {0}/{1}", bossCount, bossMax)),
				new DSPair(Math.Min((double)bossWinCount / bossWinMax, 1.0), string.Format("BOSS 胜 {0}/{1}", bossWinCount, bossWinMax))
			};
