	...
	public static class BattleDetailDescriptor
	{
		...
		public static string GetBattleDetail(BattleData battle)
		{
			...
			foreach (var phase in battle.GetPhases())
			{
				...
				else if (phase is PhaseNightBattle)
				{
...

					{
						...
						if (eq != null)
						{
							sb.Append("自军夜间触接 : ").AppendLine(eq.Name);
						}
						eq = KCDatabase.Instance.MasterEquipments[p.TouchAircraftEnemy];
						if (eq != null)
						{
							sb.Append("敌军夜间触接 : ").AppendLine(eq.Name);
						}
					}
				...
				else if (phase is PhaseSearching)
				{
					...
					sb.Append("自军阵形 : ").Append(Constants.GetFormation(p.FormationFriend));
					sb.Append(" / 敌军阵形 : ").AppendLine(Constants.GetFormation(p.FormationEnemy));
					sb.Append("交战形态 : ").AppendLine(Constants.GetEngagementForm(p.EngagementForm));
					sb.Append("自军索敌 : ").Append(Constants.GetSearchingResult(p.SearchingFriend));
...

			{
				...
				if (friendescort != null)
					sbmaster.AppendLine("〈己方主力舰队〉");
				else
					sbmaster.AppendLine("〈己方舰队〉");
				...
				if (friendescort != null)
				{
					sbmaster.AppendLine().AppendLine("〈己方随伴舰队〉");
				...
				sbmaster.AppendLine();
				if (enemyescort != null)
					sbmaster.AppendLine("〈敌主力舰队〉");
				else
					sbmaster.AppendLine("〈敌舰队〉");
				...
				if (enemyescort != null)
				{
					sbmaster.AppendLine().AppendLine("〈敌随伴舰队〉");
		...
		private static void GetBattleDetailPhaseAirBattle(StringBuilder sb, PhaseAirBattleBase p)
		{
			...
			if (p.IsStage2Available)
			{
				...
				{
					sb.AppendFormat("对空 CI ( {0}, {1}({2}) )", p.AACutInShip.NameWithLevel, Constants.GetAACutinKind(p.AACutInKind), p.AACutInKind);
				}
				sb.AppendLine();
				sb.AppendFormat("　自军 : -{0}/{1}\r\n　敌军 : -{2}/{3}\r\n",
		...
		private static void OutputFriendData(StringBuilder sb, FleetData fleet, int[] initialHPs, int[] maxHPs)
		{
			...
			for (int i = 0; i < fleet.MembersInstance.Count; i++)
			{
				...
				sb.AppendFormat("#{0}: {1} {2} HP: {3} / {4} - 火力 {5}, 雷装 {6}, 对空 {7}, 装甲 {8}{9}\r\n",
					i + 1,
		...
		private static void OutputFriendBase(StringBuilder sb, int[] initialHPs, int[] maxHPs)
		{
			...
			for (int i = 0; i < initialHPs.Length; i++)
			{
				...
				sb.AppendFormat("#{0}: 陆上设施 第 {1} 基地 HP: {2} / {3}\r\n\r\n",
					i + 1,
					i + 1,
					initialHPs[i], maxHPs[i]);
			}
		...
		private static void OutputEnemyData(StringBuilder sb, ShipDataMaster[] members, int[] levels, int[] initialHPs, int[] maxHPs, EquipmentDataMaster[][] slots, int[][] parameters)
		{
			...
			for (int i = 0; i < members.Length; i++)
			{
				...
				if (parameters != null)
				{
					sb.AppendFormat(" - 火力 {0}, 雷装 {1}, 对空 {2}, 装甲 {3}",
		...
		private static string GetBattleResult(BattleManager bm)
		{
			...
			sb.AppendLine("◆ 战斗结果 ◆");
			sb.AppendFormat("等级 : {0}\r\n", result.Rank);

			if (bm.IsCombinedBattle)
			{
				sb.AppendFormat("MVP ( 主力舰队 ) : {0}\r\n",
					result.MVPIndex == -1 ? "(なし)" : bm.FirstBattle.Initial.FriendFleet.MembersInstance[result.MVPIndex - 1].NameWithLevel);
				sb.AppendFormat("MVP ( 随伴舰队 ) : {0}\r\n",
					result.MVPIndexCombined == -1 ? "( 无 )" : bm.FirstBattle.Initial.FriendFleetEscort.MembersInstance[result.MVPIndexCombined - 1].NameWithLevel);

			}
			else
			{
				sb.AppendFormat("MVP: {0}\r\n",
					result.MVPIndex == -1 ? "( 无 )" : bm.FirstBattle.Initial.FriendFleet.MembersInstance[result.MVPIndex - 1].NameWithLevel);
			}
