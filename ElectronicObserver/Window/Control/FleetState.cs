		...
		public void UpdateFleetState(FleetData fleet, ToolTip tooltip)
		{
			...
			else
			{
				...
				if (!fleet.IsInSortie && fleet.MembersWithoutEscaped.Any(s => s != null && s.HPRate <= 0.25 && s.RepairingDockID == -1))
				{
					...
					state.SetInformation(FleetStates.Damaged, "存在大破舰！", "存在大破舰！", (int)ResourceManager.IconContent.FleetDamaged,
				...
				//入渠中
				{
					...
					{   //入渠中
						...
						tooltip.SetToolTip(state.Label, "结束时间 : " + DateTimeHelper.TimeToCSVString(state.Timer));
