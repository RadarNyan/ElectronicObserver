			...
			if ( phase1Enabled && phase1.IsStage1Available ) {
				...
				if ( needAppendInfo ) {
					...
					if ( phaseJetStage1Enabled )
						sb.Append( "喷气式战 : " ).AppendLine( Constants.GetAirSuperiority( phaseJet.AirSuperiority ) );

					sb.Append( "第 1 次 : " ).AppendLine( Constants.GetAirSuperiority( phase1.AirSuperiority ) );

					if ( phase2Stage1Enabled )
						sb.Append( "第 2 次 : " ).AppendLine( Constants.GetAirSuperiority( phase2.AirSuperiority ) );
				...
				if ( jetTouchFriend > 0 || phase1TouchFriend > 0 || phase2TouchFriend > 0 ) {
					...
					if ( phaseJetStage1Enabled )
						sb.AppendFormat( "喷气式战 : {0}\r\n", jetTouchPlane != null ? jetTouchPlane.Name : "( 无 )" );
					if ( needAppendInfo )
						sb.Append( "第 1 次 : " );
					sb.AppendFormat( "{0}\r\n", phase1TouchPlane != null ? phase1TouchPlane.Name : "( 无 )" );
					if ( phase2Stage1Enabled )
						sb.AppendFormat( "第 2 次 : {0}\r\n", phase2TouchPlane != null ? phase2TouchPlane.Name : "( 无 )" );
		...
		private void SetHPBar( BattleData bd ) {
			...
			for ( int i = 0; i < 6; i++ ) {
				if ( initialHPs[i] != -1 ) {
					...
					if ( isEscaped ) {
						bar.BackColor = Utility.Configuration.Config.UI.Battle_ColorHPBarsEscaped;
					} else {
						bar.BackColor = Utility.Configuration.Config.UI.BackColor;
					}
					bar.RepaintHPtext();
				}
			}
