...
namespace ElectronicObserver.Window
{
	...
	public partial class FormBattle : DockContent
	{
			...
			if (phase1Enabled && phase1.IsStage1Available)
			{
				...
				if (needAppendInfo)
				{
					...
					if (phaseJetStage1Enabled)
						sb.Append("喷式战 : ").AppendLine(Constants.GetAirSuperiority(phaseJet.AirSuperiority));
				...
				if (jetTouchFriend > 0 || phase1TouchFriend > 0 || phase2TouchFriend > 0)
				{
					...
					if (phaseJetStage1Enabled)
						sb.AppendFormat("喷式战 : {0}\r\n", jetTouchPlane?.Name ?? "( 无 )");
					if (needAppendInfo)
						sb.Append("第 1 次 : ");
					sb.AppendFormat("{0}\r\n", phase1TouchPlane?.Name ?? "( 无 )");
					if (phase2Stage1Enabled)
						sb.AppendFormat("第 2 次 : {0}\r\n", phase2TouchPlane?.Name ?? "( 无 )");
		...
		private void SetHPBar(BattleData bd)
		{
			...
			HPBars[BattleIndex.Get(BattleSides.EnemyMain, 0)].RepaintHPtext();
