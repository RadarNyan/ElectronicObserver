	...
	public class powerup : APIBase
	{
		...
		public override void OnRequestReceived(Dictionary<string, string> data)
		{
			...
			foreach (string id in data["api_id_items"].Split(",".ToCharArray()))
			{
				...
				Utility.Logger.Add(2, "", "已除籍 : ", ship.NameWithLevel);
		...
		public override void OnResponseReceived(dynamic data)
		{
			...
			if (ship != null)
			{
				...
				if (Utility.Configuration.Config.Log.ShowSpoiler)
				{
					...
					{
						Utility.Logger.Add(2, "", "对 ", ship.NameWithLevel, " 的近代化改修失败了。");
					...
					else
					{
						...
						if (aa > 0)
							contents.AddLast(string.Format("对空 +{0} ({1}/{2})", aa, updated_ship.AABase, updated_ship.MasterShip.AAMax));
						...
						if (asw > 0)
							contents.AddLast(string.Format("对潜 +{0} ({1}/{2})", asw, updated_ship.ASWBase, updated_ship.ASWMax + updated_ship.MasterShip.ASWModernizable));

						int luck = updated_ship.LuckBase - ship.LuckBase;
						if (luck > 0)
							contents.AddLast(string.Format("运 +{0} ({1}/{2})", luck, updated_ship.LuckBase, updated_ship.MasterShip.LuckMax));
